#!/bin/bash
#########################################################################################################
# Benchmark     :       DockerCheck.sh
# Version       :       V-1.0
# Creator       :       Jeremy Huang
# Date          :       2022-10-8
# Category      :       Docker Monitoring
#########################################################################################################
# Description   :       This script is used to complete the output of the docker stats command.
...
docker stats | while read line; do
    ...
    dat=$(date)
    echo "Date & Time: $dat" >>/tmp/emailFiles/Docker-Resources-Usage-Stats.txt
    ...
    for i in $dStats; do
        ...
        if [ $cpuPerc -ge 100 ] || [ $memPerc -ge 50 ]; then
            #IFS=$oldifs
    ...
    SUM_RAM=${SUM_RAM/.*/}
    SUM_CPU=${SUM_CPU/.*/}
    if [ $SUM_RAM -ge 70 ] && [ $SUM_CPU -ge 100 ]; then #if [ $SUM_RAM -ge 70 ] || [ $SUM_CPU -ge 100  ]
        echo " " >>/tmp/emailFiles/Docker-Resources-Usage-Stats.txt
